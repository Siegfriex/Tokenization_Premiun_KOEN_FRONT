        ...
        <ArticleFullWidthBreak className="space-y-12 my-8">
          ...
          <div className="bg-surface-alt border border-rule rounded-xs p-6 sm:p-10 space-y-8 shadow-xs">
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-xs font-mono">
                <span className="text-ink-body font-semibold">반복 횟수 (Prompt Iterations):</span>
                <span className="text-ink font-bold text-sm font-mono">
            ...
            <div className="pt-4 border-t border-rule space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center md:text-left">
                <div className="space-y-1">
                  <span className="text-xs font-mono text-ink-muted uppercase">English Baseline Tokens</span>
                  <div className="text-3xl sm:text-4xl font-mono font-bold text-ink-muted">
                    {totalEn.toLocaleString()}
                    <span className="text-xs font-normal text-ink-subtle ml-1.5 font-sans">tok</span>
                  </div>
                </div>

                <div className="space-y-1">
                  <span className="text-xs font-mono text-ink-muted uppercase">Hangul Cumulative Tokens</span>
                  <div className="text-3xl sm:text-4xl font-mono font-bold text-ink">
                    {totalKo.toLocaleString()}
                    <span className="text-xs font-normal text-ink-muted ml-1.5 font-sans">tok</span>
                  </div>
                </div>

                <div className="space-y-1 md:border-l md:border-rule md:pl-6">
                  <span className="text-xs font-mono text-ink uppercase font-bold tracking-wider">
                    ACCUMULATED BURDEN GAP
                  </span>
                  <div className="text-4xl sm:text-5xl font-mono font-black text-ink">
                    +{totalGap.toLocaleString()}
                    <span className="text-xs font-normal text-ink-muted ml-1.5 font-sans">tokens</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-5 bg-surface border border-rule rounded-xs font-mono text-xs max-w-lg mx-auto md:mx-0 space-y-3">
                <div className="flex items-center justify-between border-b border-dashed border-rule pb-2 text-[11px]">
                  <span className="font-bold text-ink tracking-wider uppercase">
                    TOKEN RECEIPT (토큰 사용 명세서)
                  </span>
                  <span className="text-ink-muted">{promptCount.toLocaleString()} ITERATIONS</span>
                </div>
                <div className="space-y-1.5 text-xs">
                  <div className="flex justify-between">
                    <span className="text-ink-body">KOREAN ({promptCount}회)</span>
                    <span className="font-bold text-ink">{totalKo.toLocaleString()} TOKENS</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-ink-muted">ENGLISH ({promptCount}회)</span>
                    <span className="text-ink-muted">{totalEn.toLocaleString()} TOKENS</span>
                  </div>
                </div>
                <div className="border-t border-rule-strong pt-2 flex justify-between font-bold text-sm text-ink">
          ...
          <div className="space-y-8">
            <div className="border-b border-rule pb-3 flex items-center justify-between">
              ...
              <span className="text-xs font-mono text-ink-muted">AI Exposure vs. Language Intensity</span>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Cluster 1: Engineering / Technical */}
              <div className="bg-surface-alt border border-rule rounded-xs p-6 sm:p-8 space-y-6 flex flex-col justify-between">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2 text-xs font-mono">
                    <div className="flex justify-between py-1 border-b border-rule/60">
                      <span className="text-ink-muted">AI Exposure Level:</span>
                      <span className="font-bold text-ink">{engineeringData.aiExposureLevel}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-rule/60">
                      <span className="text-ink-muted">Language Intensity:</span>
                    ...
                    <div className="space-y-1.5">
                      {engineeringData.occupations.map((occ, idx) => (
                        ...
                        >
                          ...
                          <span className="text-[10px] font-mono px-2 py-0.5 bg-surface-alt text-ink-body rounded-xs border border-rule">
                ...
                <div className="p-3 bg-surface border border-rule rounded-xs text-xs text-ink-muted font-mono">
                  평가: 코드 및 영문 토큰 비중으로 인해 상대적 토큰 페널티 완충
                </div>
              </div>

              {/* Cluster 2: Social Science / Knowledge-intensive */}
              <div className="bg-surface border-2 border-rule-strong rounded-xs p-6 sm:p-8 space-y-6 flex flex-col justify-between shadow-xs">
                <div className="space-y-4">
                  <div className="flex items-center justify-between border-b border-rule pb-3">
                    <div className="flex items-center gap-2">
                ...
                <div className="p-3 bg-surface-alt border border-rule rounded-xs text-xs text-ink font-mono font-semibold">
